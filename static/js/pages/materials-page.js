...
async function loadMaterials() {
  ...
  try {
    ...
    tbody.innerHTML = emptyRow(4, 'Error loading materials');
  }
}

function renderMaterials(materials) {
  const tbody = document.getElementById('materials-body');
  if (!materials.length) { tbody.innerHTML = emptyRow(4); return; }
...
document.getElementById('materials-search').addEventListener('input', function () {
  ...
  renderMaterials(allMaterials.filter(m => m.name.toLowerCase().includes(q) || m.unit.toLowerCase().includes(q)));
});
...
function deleteMaterial(id, name) {
  confirmAction('Delete Material', `Delete "${name}"?`, async () => {
    try {
